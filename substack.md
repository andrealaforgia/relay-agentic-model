...
EDD draws a sharp line that matters here: **evidence is a concrete demonstration of the running system's behaviour** — real inputs paired with their real outputs — *not* a passing test. A test the Builder wrote and watched go green is, in EDD's own words, "a second assertion by the same entity that made the first assertion." So the Examiner demands the system *actually doing the thing* (executed evidence) and treats narrated, would-happen claims (generative evidence) as weak. Automated tests still matter — but as the Builder's own discipline and regression net, not as the proof shown here (more in the Builder section). The Examiner is the adversarial reviewer; the ledger records what was demonstrated.

### 5. The Builder — the only one who touches code

The Builder is the implementer, and the only agent that writes and runs real code. It works **test-first**, by Test-Driven Development: for each expectation it writes a failing automated test (Red), writes the minimum code to make it pass (Green), then refactors — never writing production code that isn't driven by a failing test. Those tests are its regression net (what EDD calls *stabilising*). But its **evidence** is something distinct: it actually *runs the real system* and shows the concrete input→output behaviour that proves each expectation — a captured run, a measured value, a screenshot — never "my tests pass," which would just be grading its own homework. It reports back **which expectations are fulfilled** — and *only* that, never implementation detail; "I used a hash map keyed by cell coordinates" is a contract violation. The correct report is "E2 now holds; here is the run that shows it." (EDD is the cross-agent contract — *what* must hold, proven by demonstration; TDD is *how* the Builder gets there.)
...
2. **Expectation-Driven Development at the core**, so correctness is proven by plain-language expectations and **demonstrated, executed evidence** — the system shown actually doing the thing, judged adversarially by the Examiner. The Builder works test-first (TDD) as its implementation discipline and regression net, but a passing test is never a substitute for the demonstration.
