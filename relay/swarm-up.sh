...
# starts the 8 communication daemons, fully detached from this shell (they
...
nohup python3 "$TOOL_DIR/iterm_progress.py"    --home "$RELAY_HOME" >> "$RELAY_HOME/progress-trigger.log"  2>&1 & disown

sleep 1
n=$(pgrep -f "relay/(iterm_dispatch|dispatch_watchdog|iterm_sentinel|iterm_qa|iterm_warden|iterm_reaper|iterm_courier|iterm_progress)\.py --home $RELAY_HOME" | wc -l | tr -d ' ')
echo
echo "swarm '$SWARM_NAME' is up: $PROJECT_DIR  ($n/8 daemons running)"
